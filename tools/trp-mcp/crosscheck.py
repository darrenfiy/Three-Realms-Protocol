...
source_integrity 守的是既有宣告：CASE 為來源檔記過 bytes 與 SHA-256，
而在此之前沒有任何程式讀過那些雜湊。新的來源不再記指紋（進庫後有沒有被改
由 git 回答），所以這個檢查的涵蓋範圍會逐年縮小，那是對的方向。
它只回報哪幾筆需要人去讀，不替語義判讀簽名——理由與處置順序見
AGENTS.md〈語義在指紋之前〉。
