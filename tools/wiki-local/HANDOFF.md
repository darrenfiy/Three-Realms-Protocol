...
- `add-local-host-alias.ps1`: optional elevated helper that adds `wiki.three-quarters.test` to the local Windows hosts file
...
- If you want something prettier than `localhost` before public deployment, the optional local alias is `http://wiki.three-quarters.test`. That is machine-local only and depends on the Windows hosts file.
