...
source:
  ...
  mechanical_checks:
    - META-088 附件 sha256 與 bytes 與 frontmatter 完全一致（9E0D…414 / 21521）（該欄位已於 2026-09-20 隨指紋退場，見 CASE·META-134；原值在 git 歷史）。
...
META-088:
  判定: 成立。
  依據: 附件 sha256／bytes 驗證通過（該欄位已於 2026-09-20 隨指紋退場，見 CASE·META-134；原值在 git 歷史）；引句與發話者歸屬逐字對回原文；
