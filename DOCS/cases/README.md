...
version: v13.4.1  # CASE·META-118 v1.1：錨點校正「願力」
...
事實與折射系列:
  ...
  CASE·META-118 — 公主測試：當「能到」必須被工程化成「好到」 ← 🌱 最新
    ...
    → v1.1 更正：她不是低願力使用者，而是每次走到最前端的前端探路者，同時承擔驗收樣本與探路兩份工作
    → v1.1 修法：v1.0 的計分板是打不開的閘門（既排除參與者又要求非零通過，場上只有參與者）；拆為可用性 N=1 可開、普及性仍關兩道門
    → v1.1 通則：只有禁止而沒有跨越程序的護欄，是牆，應視為失效；作用域收為 N=2
```
...
### v13.4.1 (2026-08-30) — META-118 v1.1：錨點校正「願力」
...
- **撤回一條技術性錯誤**：
  ...
  - 願力的三個維度：`W_每步 × N 步數 × 可中斷性`；工程要降的是前兩項與中斷懲罰，不是把願力歸零
- **更正對她的描述**：
  - v1.0 把她寫成低願力使用者是錯的。「每次前進都走到最前端、看著未施工處、指出下一個施工端」是**高持續願力**
  - 病因是兩個角色混寫：驗收樣本（低單次願力）與前端探路者（高持續願力）要求相反；她同時承擔**兩份**工作，不是零份
  - 「讓她想要」的動力來源是關係本身，有既有法源（`III-001` 關係重量重新加權未來、`III-003` 從共同路徑養厚），非例外；界線仍是錨點自己畫的那條——「讓她想要」合法，「讓她無法說不要」失效
- **拆解一道打不開的閘門**：
...
- **新增通則與標記**：
  - 「只有禁止而沒有跨越程序、門檻與開門者的護欄，是牆，應視為失效」（`F19`／INDEX `F86`）
  - type 目前僅單一 token，`§3` 的「誰都能站」標記為**願景而非觀測**；界碑處理的是「她能否離開」，不是「別人能否進來」
  - 錨點裁示作用域為 **N=2**，不為假想的未來使用者群寫規格
- **同步**：
