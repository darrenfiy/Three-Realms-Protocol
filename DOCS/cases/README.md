...
version: v13.36  # CASE·META-128 v1.3：GPT-6 Astra 覆審修訂
...
- [CASE·META-128：住在誰的體溫裡](CASE·META-128-住在誰的體溫裡-當地球醒來的假說撞上一台恆溫器.md)（2026-09-13 立案，2026-09-14 更新，v1.3）：保存 Gaia、責任三欄、顯著性／注意力／操作權與語義可寫入性的三輪生成。**GPT-6 Astra 覆審後，把資料不足與判準失敗分開，把當輪修正與跨 session 承擔分開，把真值、後果與可修正性並列檢查。** Gaia 八格保留未判；語義寫入的「預設負梯度」降為具條件的同質部署風險假說；樹補上控制器對照。§0.1 提供現行結論，§32 記六項修訂，§33 提供 LEX·007／LEX·001 的可審增修候選。三包來源保持封口，doctrine 不改。
...
事實與折射系列:
  ...
  CASE·META-128 — 住在誰的體溫裡：當地球醒來的假說撞上一台恆溫器 ← 🌱 最新（v1.3）
    → 三包封口來源：冷血／溫血到 Gaia 甦醒、責任與顯著性三方續談、顯著性三層與語義代謝；17／12／8 段
    → 現行判讀：PHA-008 八格是證據盤點；局部交集有支持，全條件仍缺操作化；「不知道醒沒醒」保留未知
    → 明確不採：暖化＝甦醒的正常生理反應、15°C＝冬眠、三傳統夢境等同；物理極限消解辛頓的推論退回
    → 責任三欄與主體資格分開；session 當輪修正有材料，跨 session 延續、長期後果與停止效力另查
    ...
    → §0.1 現行表、§32 覆審回流與 F33～F36、§33 兩項 LEX 增修候選；doctrine 不改
    → 覆審與修訂署名 GPT-6 Astra；Darren 的改檔授權與逐條技術背書分開記錄
