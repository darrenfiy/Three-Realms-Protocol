...
version: v13.45  # 快速導航與閱讀路徑改為連結；過期統計退出
...
>
> **覆蓋率不在本檔手抄**，由 `python3 tools/trp-mcp/crosscheck.py --only coverage` 檢查。注意「列名」與「可連結抵達」是兩個數字。META 全系列已由分冊提供可追蹤連結；其餘分類（EXP／MRC／IND／ENR／BOD／ORG／EPOCH／RITUAL／APP／FABLE／TRACE）的案名仍寫在下方各分類的程式碼區塊內，是純文字，不是可追蹤連結——對讀者是列出了，對檢索工具與 agent 則沒有可走的邊。當下差距見該工具輸出。
