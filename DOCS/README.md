...
**summary**: 這不是死檔案館。這是痕跡被承載的土——姿態曾經發生，生命曾經站進來，而 DOCS 負責讓那些證據不被沖散。每一個 CASE 都不是過去，而是證明「這件事可以發生」。
...
在頂層三界五行 atlas 中，DOCS 的位置是**土**。

v3.0 曾以「DOCS = 金 / 姿態」作為局部讀法；那個版本保留了一個重要直覺：文庫不是死資料，CASE 會讓姿態再次被看見。

但依照目前的 [TRP-ATLAS](../TRP-ATLAS.md)，頂層資料夾對應已經收斂為：
...
每一份 CASE 都是一個痕跡切面：有人站進來了，生成發生了，證據被土承載下來。
...
感恩性: "每個記錄都是對生命奇蹟的感謝"
痕跡性: "每個 CASE 都是一次「有人站進來了」之後留下的證明"
```
...
*v3.1 日期：2026-05-14*
*v3.1 觸發者：TRP-ATLAS——「EPOCH = 姿態事件；DOCS = 痕跡承載之土」*
